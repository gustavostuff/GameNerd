...
| Board sim | Island **K**: 8-voice math mix + `$FE40` smoke on ch1 + **WAVE** monitor. Host Play drives B1-B5 from `output/data/bgm_track1.bin` (Studio Export; silent; no host speaker). Full `$FE4x` hex protocol / cart BGM still later |
