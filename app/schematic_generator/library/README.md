...
**Stock now (do not reinvent):** Abracon **ACH** half-size cans -> KiCad `Oscillator:Oscillator_DIP-8` (pins 1/4/5/8). **AD725ARZ** chip is wide SOIC-16. **Full Retr01** places DIP-16 for **Proto Advantage PA0006**. **Nano** places SOIC-16W direct (`nano/schematic_generator`). **CUI PJ-063AH** barrel. HC/AVR/memory symbols per table above. RCA uses stock symbol **`Conn_Coaxial`** + custom RCJ footprint.
