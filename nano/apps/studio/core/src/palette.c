...
/* Strong 8-bit board FG preview (resistor DAC levels, Studio-side). */
...
void r01_nano_fg_rgb(int fg, uint8_t *r, uint8_t *g, uint8_t *b) {
    int i = fg & 7;
    if (r) {
        *r = NANO_FG_RGB[i][0];
    }
...
void r01_screen_pixel_rgb(const R01Project *p, const R01World *w, const R01Screen *s, int px, int py, uint8_t *r,
    ...
    /* Nano: 1bpp — off = black backdrop, on = attr FG 0..7. */
    if (color == 0) {
        if (r) {
            *r = 0;
        }
    ...
    r01_nano_fg_rgb(r01_attr_fg(attr), r, g, b);
}
