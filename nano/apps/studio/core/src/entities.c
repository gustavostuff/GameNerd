...
void r01_entity_nano_normalize(R01EntityType *e) {
    int si;
    if (!e) {
        return;
    }
    if (e->state_count < 1) {
        e->state_count = 1;
        r01_entity_state_init(&e->states[0], r01_entity_default_state_name(0));
    }
    if (e->state_count > R01_ENTITY_STATES_MAX) {
        e->state_count = R01_ENTITY_STATES_MAX;
    }
    for (si = 0; si < e->state_count; si++) {
        ...
        memset(&st->frames[1], 0, sizeof(st->frames) - sizeof(st->frames[0]));
    }
}

int r01_entity_nano_set_state_tile(R01EntityType *e, int state_idx, int bank, int tile_id) {
    R01EntityState *st;
    R01EntityFrame *fr;
    R01EntityPart *pt;
    if (!e || state_idx < 0 || state_idx >= R01_ENTITY_STATES_MAX) {
    ...
    st = r01_entity_ensure_state(e, state_idx);
    if (!st) {
        return -1;
    }
    fr = r01_entity_ensure_frame(e, state_idx, 0);
    if (!fr) {
        return -1;
    }
    st->frame_count = 1;
...
int r01_entity_nano_get_state_tile(const R01EntityType *e, int state_idx, int *out_bank, int *out_tile_id) {
    const R01EntityState *st;
    const R01EntityFrame *fr;
    if (!e || state_idx < 0 || state_idx >= e->state_count) {
        return -1;
    }
    st = &e->states[state_idx];
    if (st->frame_count < 1 || st->frames[0].part_count < 1) {
        return -1;
    }
    fr = &st->frames[0];
    if (out_bank) {
        *out_bank = fr->parts[0].bank;
    }
    if (out_tile_id) {
        *out_tile_id = fr->parts[0].tile_id;
    }
    return 0;
}
