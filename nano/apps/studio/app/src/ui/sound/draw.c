...
static const char *const k_ch_label[UI_SOUND_BGM_CH] = {"Music"};
...
void draw_sound_editor(UiState *ui, SDL_Renderer *r) {
    ...
    /* Nano: one Music lane — show role text, no isolate radios. */
    {
        int y = lo.timeline_y + (lo.lane_h - 8) / 2;
        font_draw(r, lo.insp_x, y, k_ch_role[0], 160, 160, 170);
    }
}
