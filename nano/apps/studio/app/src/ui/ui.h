...
/* Nano FG picker: 2 rows × 4 colors (board FG 0..7). */
#define UI_FG_COLS 4
#define UI_FG_ROWS 2
#define UI_FG_CELL UI_PAL_GRID_CELL
#define UI_FG_GRID_W (UI_FG_COLS * UI_FG_CELL)
#define UI_FG_GRID_H (UI_FG_ROWS * UI_FG_CELL)
...
#define UI_WORLDS_BODY_H (UI_BTN_H + UI_WORLD_VIEW)
...
#define UI_BANKS_BODY_H (UI_BTN_H + UI_BANKS_GRID)
...
#define UI_SOUND_BGM_CH 1 /* Nano: single Music (Pulse) lane */
...
#define UI_ACC_BANKS 1
#define UI_ACC_ENTITIES 2
...
#define UI_ACC_SECTIONS 3

/* Legacy aliases (removed from Nano UI). */
#define UI_ACC_PALS (-2)
#define UI_ACC_METATILES (-2)
#define UI_ACC_METASPRITES (-2)
#define UI_ACC_SPRITES UI_ACC_BANKS
...
#define UI_ENTITY_MODAL_W (UI_TILE_CANVAS + UI_UNIT * 4)
#define UI_METASPRITE_MODAL_H 304
#define UI_ENTITY_MODAL_H 328
#define UI_ENTITY_BANK_GRID 128 /* metasprite modal legacy */
#define UI_ENTITY_COMPOSE 128   /* metasprite modal legacy */
...
typedef struct UiTileEdit {
    int open;
    int pal;   /* Nano FG color 0..7 */
    int color; /* paint ink 0/1 (1bpp) */
    int tile_id;
    int bank;
    int flip_h;
    int flip_v;
...
typedef struct UiEntityEdit {
    ...
    int paint_on; /* 1 bpp paint: 1 = FG pixel, 0 = clear */
    int dragging; /* 0 none, 1 paint stroke */
} UiEntityEdit;
...
typedef struct UiBrush {
    ...
    int bank;
    int tile_id;
    int pal; /* Nano: FG color 0..7 (attr bits) */
    int flip_h;
    int flip_v;
    uint8_t chr[R01_TILE_BYTES];
} UiBrush;
...
static inline int ui_preview_x(const UiState *ui) {
    ...
    int sw = ui_screen_w(ui);
    int x = UI_SIDEBAR_W + (pane - sw) / 2;
    if (x < UI_SIDEBAR_W) {
        x = UI_SIDEBAR_W;
    }
    if (x + sw > ui_ctrl_x(ui)) {
        x = ui_ctrl_x(ui) - sw;
        if (x < UI_SIDEBAR_W) {
            x = UI_SIDEBAR_W;
        }
