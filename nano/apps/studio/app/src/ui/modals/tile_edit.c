...
void tile_edit_open(UiState *ui, int tx, int ty) {
    ...
    if (s && w && tx >= 0 && ty >= 0) {
        ...
        ui->tile_edit.pal = r01_attr_fg(attr);
...
void draw_tile_modal(UiState *ui, SDL_Renderer *r) {
    ...
    draw_label(r, lo.pal_x, lo.pal_label_y, "Color");
    ui_fg_strip_draw(r, lo.pal_x, lo.pal_y, ui->tile_edit.pal);

    r01_nano_fg_rgb(ui->tile_edit.pal, &fr, &fg, &fb);
    ...
    for (sy = 0; sy < 8; sy++) {
        for (sx = 0; sx < 8; sx++) {
            ...
            int cell = 16;
            if (col != 0) {
                fill_rect(r, lo.canvas_x + sx * cell, lo.canvas_y + sy * cell, cell - 1, cell - 1, fr, fg, fb);
            }
        }
    }
...
int tile_modal_handle(UiState *ui, int lx, int ly, int down, Uint8 button) {
    ...
    if (ui_fg_strip_hit(lx, ly, lo.pal_x, lo.pal_y, &fg)) {
        ui->tile_edit.pal = fg & 7;
        ui->tile_edit.color = 1;
        return 1;
    }
    if (lx >= lo.canvas_x && lx < lo.canvas_x + UI_TILE_CANVAS && ly >= lo.canvas_y &&
        ...
        ink = (button == SDL_BUTTON_RIGHT) ? 0 : 1;
        ui->tile_edit.color = ink;
        if (ui->keys[SDL_SCANCODE_F]) {
            r01_tile_flood_fill(ui->tile_edit.chr, sx, sy, (uint8_t)ink);
        } else {
            r01_tile_set_pixel(ui->tile_edit.chr, sx, sy, (uint8_t)ink);
        }
        return 1;
    }
