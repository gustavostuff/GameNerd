...
void ui_fg_strip_draw(SDL_Renderer *r, int x, int y, int sel_fg) {
    int i;
    if (!r) {
        return;
    }
    ...
    for (i = 0; i < 8; i++) {
        uint8_t cr, cg, cb;
        int cx = x + (i % UI_FG_COLS) * UI_FG_CELL;
        int cy = y + (i / UI_FG_COLS) * UI_FG_CELL;
        r01_nano_fg_rgb(i, &cr, &cg, &cb);
        fill_rect(r, cx, cy, UI_FG_CELL - 1, UI_FG_CELL - 1, cr, cg, cb);
        if (i == sel_fg) {
            draw_rect(r, cx, cy, UI_FG_CELL - 1, UI_FG_CELL - 1, 255, 255, 255);
        }
    }
}

int ui_fg_strip_hit(int lx, int ly, int x, int y, int *out_fg) {
    int col, row;
    if (lx < x || lx >= x + UI_FG_GRID_W || ly < y || ly >= y + UI_FG_GRID_H) {
        return 0;
    }
    col = (lx - x) / UI_FG_CELL;
    row = (ly - y) / UI_FG_CELL;
    if (col < 0 || col >= UI_FG_COLS || row < 0 || row >= UI_FG_ROWS) {
        return 0;
    }
    if (out_fg) {
        *out_fg = row * UI_FG_COLS + col;
    }
    return 1;
}
