...
/*
 * Nano motion: pixel_x/y integrate speed (and later physics). tile_x/y are derived
 * for MAP/draw. The picture always stamps the entity on its tile cell — never at
 * sub-tile pixel offsets (nano/docs/graphics.md).
 */
typedef struct R01nePlay {
    int enabled;
    int player_px; /* internal integration */
    int player_py;
    int player_tx; /* floor(px/8) — draw / solid / screen */
    int player_ty;
